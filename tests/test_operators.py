...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    # Crosses 0.5 at 0
    if abs(a) < 1e-5:  # Check if close to zero
        assert_close(sigmoid(a), 0.5)
    
    # Strictly increasing (test with small delta)
    if a > -10.0 and a < 10.0:  # Avoid extreme values
        delta = 1e-5
        assert sigmoid(a + delta) > sigmoid(a - delta)
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    sum1 = sum(ls1)
    sum2 = sum(ls2)
    total_sum = add(sum1, sum2)
    
    element_wise_sum = sum(addLists(ls1, ls2))
    
    assert_close(total_sum, element_wise_sum)
